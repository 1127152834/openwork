...
export function validateMcpServerName(name: string): string {
  ...
  if (!trimmed) {
    throw new Error(t("mcp.name_required", currentLocale()));
  }
  if (trimmed.startsWith("-")) {
    throw new Error(t("mcp.server_name_no_dash_prefix", currentLocale()));
  }
  if (!/^[A-Za-z0-9_-]+$/.test(trimmed)) {
    throw new Error(t("mcp.server_name_invalid_format", currentLocale()));
  }
  return trimmed;
}
...
export async function removeMcpFromConfig(
  ...
  if (!writeResult.ok) {
    throw new Error(writeResult.stderr || writeResult.stdout || t("mcp.config_write_failed", currentLocale()));
  }
}
