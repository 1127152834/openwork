  ...
  return (
    <section class="space-y-6 animate-in fade-in duration-300">
      <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div class="space-y-1">
          <h2 class="text-3xl font-bold text-dls-text">{translate("extensions.title")}</h2>
          <p class="text-sm text-dls-secondary mt-1.5">
            {translate("extensions.subtitle")}
          </p>
            ...
            <Show when={connectedAppsCount() > 0}>
              <div class="inline-flex items-center gap-2 rounded-full bg-green-3 px-3 py-1">
                ...
                <span class="text-xs font-medium text-green-11">
                  {translateWithVars("extensions.connected_apps", {
                    count: connectedAppsCount(),
                    suffix: connectedAppsCount() === 1 ? "" : "s",
                  })}
                </span>
              </div>
            </Show>
            <Show when={pluginCount() > 0}>
              <div class="inline-flex items-center gap-2 rounded-full bg-gray-3 px-3 py-1">
                ...
                <span class="text-xs font-medium text-gray-11">
                  {translateWithVars("extensions.plugins_count", {
                    count: pluginCount(),
                    suffix: pluginCount() === 1 ? "" : "s",
                  })}
                </span>
              </div>
            </Show>
        ...
        <div class="flex flex-wrap items-center gap-2">
          <div class="flex items-center gap-2">
            ...
            >
              {translate("extensions.filter_all")}
            </button>
            <button
              type="button"
            ...
            >
              <Box size={14} />
              {translate("extensions.filter_apps")}
            </button>
            <button
              type="button"
            ...
            >
              <Cpu size={14} />
              {translate("extensions.filter_plugins")}
            </button>
          </div>
      ...
      <Show when={section() === "all" || section() === "mcp"}>
        <div class="space-y-4">
          <div class="flex items-center gap-2 text-sm font-medium text-gray-12">
            ...
            <span>{translate("extensions.section_apps")}</span>
          </div>
          <McpView
            showHeader={false}
            busy={props.busy}
      ...
      <Show when={section() === "all" || section() === "plugins"}>
        <div class="space-y-4">
          <div class="flex items-center gap-2 text-sm font-medium text-gray-12">
            ...
            <span>{translate("extensions.section_plugins")}</span>
          </div>
          <PluginsView
            busy={props.busy}
            activeWorkspaceRoot={props.activeWorkspaceRoot}
